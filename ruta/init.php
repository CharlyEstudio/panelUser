	...
	if($encRow){
		if(is_numeric($getParamUser)){
			...
			try {
				$numRow 		= $db->NumRows();
				if($numRow) {
					$checUser 	= "SELECT c.numero, c.clienteid as usuarioID, c.saldo, c.limite, c.vendedorid, p.nombre as vendedor, c.diacredito, c.diavis, d.domid, d.nombre, d.rfc, c.lista, def.dpnombre, c.ignorasuspender, c.activo, co.correo, usu.password as pas2, usu.passant as pas1
									FROM cli c
			...
			$checkUser 			= "SELECT usu.clienteID, rol.rol, usu.username, usu.password, usu.clienteID
									FROM usuarios usu
