...
if(isset($_POST['user']) && isset($_POST['pass'])) {
	...
	$buscarVen = "SELECT nombre FROM per WHERE categoria = '$getParamUser'";
	$executeQuery 			= $db->Query($buscarVen);
	$perRow 				= $db->NumRows();

	if($perRow){
		$checkVendedor 		= "SELECT perid, categoria, nombre
									FROM per 
									WHERE categoria = '$getParamUser'
										AND perid = $getParamPassword";
		$buscarVendedor 	= mysqli_query($getConnection, $checkVendedor);
		$rowUser 			= mysqli_fetch_row($buscarVendedor);
		$password 			= $rowUser[0];
		$categoria 			= $rowUser[1];
		$nombre 			= $rowUser[2];
		try {
